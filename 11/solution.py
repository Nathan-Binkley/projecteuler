...
#At this point, data is a 20x20 matrix
...
def checkUpperDiag(data,i,j):
    if j - 3 >=0 and i + 3 <= 15:
        return data[i][j] * data[i+1][j-1] * data[i+2][j-2] * data[i+3][j-3]
    return 0
    
def checkLowerDiag(data,i,j):
    if i + 3 <=15 and j + 3 <= 15:
        return data[i][j] * data[i+1][j+1] * data[i+2][j+2] * data[i+3][j+3]
    return 0
    
m = 0
mL,mUD,mD,mLD = 0,0,0,0
for i in range(len(data)):
    for j in range(len(data[i])):
        try:        
            mL = checkLeft(data,i,j)
        except:
            pass
        try:
            mD = checkDown(data,i,j)
        except:
            pass
        try:
            mUD = checkUpperDiag(data,i,j)
        except:
            pass
        try:
            mLD = checkLowerDiag(data,i,j)
        except:
            pass
        if mL > m:
            m = mL
